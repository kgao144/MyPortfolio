...
const NavBar = () => {
    ...
    return (
        <header className = 'w-full px-32 pt-6 pb-8 font-medium flex items-center justify-between border-b-2 border-solid dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8'>
            ...
            <button className='flex-col items-center justify-center mt-1 hidden lg:flex py-4 rounded-full bg-white dark:bg-dark' onClick={handleClick}>
                <span className={`bg-dark dark:bg-light transition-all duration-[600ms] ease-out block h-1 w-6 rounded-sm translate-x-1 -translate-y-1 ${isOpen ? 'w-7 rotate-[315deg] translate-y-[3px]' : '-translate-y-1'}`}></span>
                <span className={`bg-dark dark:bg-light transition-all duration-500 ease-out block h-1 w-6 rounded-sm translate-y-1 -my-1 ${isOpen ? 'opacity-0' : ' opacity-100'}`}></span>
                <span className={`bg-dark dark:bg-light transition-all duration-[600ms] ease-out block h-1 w-6 rounded-sm translate-x-1 translate-y-3 ${isOpen ? 'w-7 -rotate-[315deg] translate-y-[3px]' : 'translate-y-1'}`}></span>
            </button>
            ...
            <div className='w-full flex items-center justify-between lg:hidden'>
                ...
                <nav className = "flex items-center justify-center flex-wrap">
                    ...
                    whileHover = {{y:-5}}
                    whileTap = {{scale:0.5}}
                    className = "w-11 mx-2 bg-dark text-white border-white dark:border-transparent rounded-full "
                    >
                        <div>
                            <GithubIcon />
                        </div>
...

            {
                ...
                className='min-w-[70vw] flex flex-col justify-between items-center z-30 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32'>
                    ...
                    <nav className = "flex items-center justify-center flex-wrap mt-8 pr-2">
                        <motion.a href="https://github.com/kgao144" target= {"_blank"}
                        whileHover = {{y:-5}}
                        whileTap = {{scale:0.5}}
